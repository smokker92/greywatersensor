...
int state = 0; // 0 - water flowing to sewage, 1 - water being stored
...
void Arduino_Control(){
  ...
  delay(1000);                     //we will take a reading every 800ms. You can make this much longer or shorter if you like.
   ...
   if(string_received==1){            //did we get data back from the ph Circuit?
     ...
     // print the pH reading
      lcd.setCursor(10, 0);
      lcd.print(ph_data);

//LOGIC needs to be thought through
       if((ph >= 6 && ph <= 8) && state!=1){
     Serial.println("high - start storing water\r");
     store_water();
     state = 1; //we will start storing water
     } 
     
       else if(ph >= 8 && state!=0 ){
     Serial.println("low - water to go to sewage\r");
     release_water();
     state = 0; //we will start releasing water to sewage
     }   
        else if (ph <= 6 && state!=0 ){
     Serial.println("low - water to go to sewage\r");
     release_water();
     state = 0; //we will start releasing water to sewage
     }   
...
//use valve to store this water in the clean grey water reservoir
void store_water(){
...
//return valve to open to release this water into sewage line
void release_water(){
